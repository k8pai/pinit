...
function generateData(value) {
	return { id: Math.floor(Math.random()*100000000000), url: value }
}
...
function renderElement(){
	...
	listItems = savedUrls?.map(el => (
		`<li class="mt-2 flex items-center text-blue-400 bg-transparent font-semibold text-base">
			<input class="px-3 py-1 flex-grow border-l-2 cursor-default focus:outline-none focus:ring-0 border-cyan-500/90" readonly type='text' value='${el.url}' />
			<a class='p-1 mx-2 rounded-md border-2 border-transparent hover:border-green-400/80' target='_blank' href='${el.url}'>
				<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
					<path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m13.35-.622l1.757-1.757a4.5 4.5 0 00-6.364-6.364l-4.5 4.5a4.5 4.5 0 001.242 7.244" />
				</svg>
			</a>
			<button data-url="${el.url}" class='copy-button p-1 mx-2 rounded-md border-2 border-transparent hover:border-green-400/80'>
			...
			<button data-id="${el.id}" class='delete-button p-1 mx-2 rounded-md border-2 border-transparent hover:border-green-400/80'>
...
saveInput.addEventListener("click", () => {
	...
	savedUrls = JSON.parse(localStorage.getItem('urlsaver')) || [];
	savedUrls.unshift(generateData(inpField.value));
...
saveUrl.addEventListener('click', () => {
	...
	chrome.tabs.query({active: true, currentWindow: true}, tabs => {
		...
		savedUrls = JSON.parse(localStorage.getItem('urlsaver')) || [];
		savedUrls.unshift(generateData(activeTab.url));
		localStorage.setItem('urlsaver', JSON.stringify(savedUrls));
		renderElement()
	 });
})
